...
class CalculateSrsUseCase @Inject constructor() {
    ...
    /**
     * Tính toán thời gian hiển thị dự kiến cho 4 nút (Again, Hard, Good, Easy)
     */
    fun calculateIntervalStrings(card: SrsCard): Map<String, String> {
        val hardCard = invoke(card, "hard")
        val goodCard = invoke(card, "good")
        val easyCard = invoke(card, "easy")

        return mapOf(
            "again" to "< 1m",
            "hard" to formatInterval(hardCard.interval),
            "good" to formatInterval(goodCard.interval),
            "easy" to formatInterval(easyCard.interval)
        )
    }
