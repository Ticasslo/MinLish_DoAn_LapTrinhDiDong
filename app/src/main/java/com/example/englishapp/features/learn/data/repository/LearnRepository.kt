...
class LearnRepository @Inject constructor(
    ...
    override suspend fun saveStudySession(session: com.example.englishapp.core.data.model.StudySession) {
        val entity = com.example.englishapp.core.data.local.entity.StudySessionEntity(
            sessionId = session.sessionId.ifEmpty { java.util.UUID.randomUUID().toString() },
        ...
        syncItem(
            ...
            remoteOp = {
                firebaseService.saveStudySession(session.copy(sessionId = entity.sessionId))
            },
            onSyncSuccess = {
